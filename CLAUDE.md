...
5. On every verification pass, replace `problems.md`. For `PASS`, write an explicit zero-problem report. For `FAIL` or `UNKNOWN`, write detailed problems, apply the smallest safe fix, and reverify.
...
- Verifiers must replace both `verdict.json` and `problems.md` on every pass; never leave a stale problem report behind.
